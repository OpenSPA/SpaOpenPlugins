...
class xtra(Screen, ConfigListScreen):
	...
	def strg(self):
		try:
			...
			folder_size=sum([sum([os.path.getsize(os.path.join(path_poster, fname)) for fname in files]) for path_poster, folders, files in os.walk(path_poster)])
			posters_sz = "%0.1f" % (folder_size/(1024*1024.0))
			poster_nmbr = len(os.listdir(path_poster))

			folder_size=sum([sum([os.path.getsize(os.path.join(path_banner, fname)) for fname in files]) for path_banner, folders, files in os.walk(path_banner)])
			banners_sz = "%0.1f" % (folder_size/(1024*1024.0))
			banner_nmbr = len(os.listdir(path_banner))

			folder_size=sum([sum([os.path.getsize(os.path.join(path_backdrop, fname)) for fname in files]) for path_backdrop, folders, files in os.walk(path_backdrop)])
			backdrops_sz = "%0.1f" % (folder_size/(1024*1024.0))
			backdrop_nmbr = len(os.listdir(path_backdrop))

			folder_size=sum([sum([os.path.getsize(os.path.join(path_info, fname)) for fname in files]) for path_info, folders, files in os.walk(path_info)])
